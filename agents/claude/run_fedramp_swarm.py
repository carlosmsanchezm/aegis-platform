...
def tester_options(plan: Mapping[str, Any]) -> ClaudeAgentOptions:
    ...
    return ClaudeAgentOptions(
        system_prompt=(
            "You are the Tester. Use go_build then run_tests. "
...
def failure_analyst_options(plan: Mapping[str, Any]) -> ClaudeAgentOptions:
    prompt = (
        ...
        "Inputs: diffs and failing build/test output.\n"
...
async def run_tester(plan: Plan) -> Tuple[Dict[str, Any], str]:
    prompt = (
        "1) Use go_build with target ./services/proxy/cmd/aegis-auth-proxy.\n"
        "2) Then use run_tests (use default if cmd unspecified).\n"
        "Return STRICT JSON with keys:\n"
        '{"ok":bool,"build_ok":bool,"test_ok":bool,"summaries":[str],"stdout":str,"stderr":str,'
        '"details":{"build":{...},"tests":{...}}}'
    )
    result = await ask_json(prompt, tester_options(plan.__dict__))
    timestamp = time.strftime("%Y-%m-%dT%H-%M-%SZ", time.gmtime())
    ci_path = EVIDENCE_DIR / f"{timestamp}_ci_results.json"
    ci_path.write_text(json.dumps(result, indent=2), encoding="utf-8")
    return result, str(ci_path.relative_to(REPO_ROOT))
