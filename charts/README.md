...
cd terraform
./generate-helm-values.sh --non-interactive
```
...
Layer 5: TLS_OVERRIDE_FILE            (temp: actual TLS certs)
         ↓
    Final Config: TLS enabled with certs
```

**Key insight:** `cloud.yaml` and Terraform outputs describe the infrastructure, while the generated TLS override injects the actual certificate material.
...
### Update TLS Settings
Edit the `platformApi.tls` / `proxy.tls` sections in `aegis-services/values/cloud.yaml` (and regenerate overrides as needed)
...
| `cd terraform && ./generate-helm-values.sh` | Cloud deployment with TLS |
