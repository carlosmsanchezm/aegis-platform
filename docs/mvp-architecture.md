...
The MVP consists of a control-plane service and one or more Kubernetes agents. The control-plane hosts the gRPC/REST API, maintains its catalog in a PostgreSQL database (projects, budgets, flavors, etc.), and handles workload submission. Agents run inside Kubernetes clusters, register themselves with the control plane, and periodically heartbeat GPU availability.
